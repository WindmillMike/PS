...
f = 1.5
Q_jpeg = np.clip(np.round(Q_jpeg * f), 1, 255).astype(np.int32)
...
def construieste_dqt(Q_jpeg):
    data = []
    data.extend(DQT)
    lungime = 67   # lungimea markerului
    data.extend([0x00, lungime])   # hex de precizie
    data.append(0x00)
    q_zigzag = spirala(Q_jpeg).astype(np.uint8)
    data.extend(q_zigzag)
    return data

def construieste_sof(inaltime, latime):
    data = []
    data.extend(SOF0)
    lungime = 11
...
for i in range(inaltime2 // 8):
    for j in range(latime2 // 8):
        ...
        x1 = x - 128.0
        y = dctn(x1).astype(float)
        y_impartit = np.round(y / Q_jpeg).astype(np.int32)
...
bitstream = transformare_octeti(bitstream)
sir_final = []
sir_final.extend(SOI)
sir_final.extend(construieste_sof(inaltime, latime))
