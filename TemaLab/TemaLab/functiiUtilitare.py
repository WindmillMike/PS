import numpy as np
from tabele_huffman import HUFFMAN_AC_LUMINANCE, HUFFMAN_DC_LUMINANCE, TABELE_HUFFMAN

def downsampling(canal):
    return (canal[0::2, 0::2] + canal[1::2, 0::2] + canal[0::2, 1::2] + canal[1::2, 1::2]) / 4

def transformareRGB(ycbcr):
...
def calculare_bitstream(huffman, parametrulFermecat):
    bitstream = ""
    for bloc in huffman:
        for pixel in bloc:
            if pixel[0] == "DC":
                size = pixel[1]
                dc_actual = pixel[2]
                size_dc_codat = TABELE_HUFFMAN["HUFFMAN_DC_" + parametrulFermecat].get(size)
                dc_binar = transformare_in_binar(dc_actual, size)
                bitstream += size_dc_codat + dc_binar

            elif len(pixel) == 2:
                bitstream += TABELE_HUFFMAN["HUFFMAN_AC_" + parametrulFermecat].get(pixel[1])

            elif len(pixel) == 3:
                size_ac_cu_zero = pixel[1]
                valoare = pixel[2]
                size = size_ac_cu_zero & 0x0F
                size_cu_zero_codat = TABELE_HUFFMAN["HUFFMAN_AC_" + parametrulFermecat].get(size_ac_cu_zero)
